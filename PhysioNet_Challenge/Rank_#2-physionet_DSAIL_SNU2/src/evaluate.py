# PhysioNet/Computing in Cardiology Challenge 2020 (https://github.com/physionetchallenges/evaluation-2020)
#!/usr/bin/env python
...
#
#   python evaluate.py labels outputs scores.csv
#
...
# Load weights.
def load_weights(weight_file, classes=None):
    # Load the table with the weight matrix.
    rows, cols, values = load_table(weight_file)
    rows_list = [row.split('|')[0] for row in rows]
    ...
    if classes is None:
        ...
        for i, a in enumerate(rows_list):
            if a in classes:
                k = classes.index(a)
                for j, b in enumerate(rows_list):
                    if b in classes:
                        l = classes.index(b)
...
# Find best class-specific thresholds
def find_best_threshold(weights, labels, outputs, classes, normal_class, idx):
    outputs_temp = outputs > 0.5
    best_score = compute_challenge_metric(weights, labels, outputs_temp, classes, normal_class)
    best_threshold = 0.5

    for j in np.arange(0.00, 1.00, 0.01):
        outputs_temp[:, idx] = outputs[:, idx] > j
        score = compute_challenge_metric(weights, labels, outputs_temp, classes, normal_class)
        if score > best_score:
            best_score = score
            best_threshold = j
